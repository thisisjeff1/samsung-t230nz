...
unsigned int ccr_lpm;
EXPORT_SYMBOL(ccr_lpm);
static int ccr_lpm_check(char *str)
{
	if (strncmp(str, "seccharger", 7) == 0)
		ccr_lpm = 1;
...
__setup("androidboot.mode=", ccr_lpm_check);
#endif
...
#ifdef CONFIG_USB_ANDROID_SAMSUNG_CCR_PROTOCOL
			if (ccr_lpm) {
				pr_info("usb: %s ccr booting_delay_sec 0s\n", __func__);
				n->booting_delay_sec = 0;
			}
#endif
			if (u_notify) {
