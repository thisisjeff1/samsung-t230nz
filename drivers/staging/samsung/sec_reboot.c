...
#ifdef CONFIG_SEC_DAIMLER_FASTBOOT
		if ((water_val.intval || wc_val.intval || (poweroff_try >= 5))) {
#else	/* !CONFIG_SEC_DAIMLER_FASTBOOT */
#ifdef CONFIG_SAMSUNG_BATTERY
		if ((ac_val.intval || water_val.intval || usb_val.intval || wc_val.intval ||
		     (poweroff_try >= 5)) && !lpcharge) {
#else	/* !CONFIG_SAMSUNG_BATTERY */
		if ((ac_val.intval || water_val.intval || usb_val.intval || wc_val.intval ||
		     (poweroff_try >= 5))) {
#endif	/* CONFIG_SAMSUNG_BATTERY */
#endif	/* CONFIG_SEC_DAIMLER_FASTBOOT */
