...
#define SENSOR_NAME_REAR  "S5K5E9_REAR"
#define SENSOR_NAME_FRONT "S5K5E9_FRONT"
...
enum
{
	SENSOR_5E9_POSITION_REAR = 0,
	SENSOR_5E9_POSITION_FRONT
};
	...
	const struct i2c_device_id *id)
{
	...
	if (ret) {
		err("position read is fail(%d)", ret);
	}

	probe_info("%s sensor_id [%d] postion [%d]\n", __func__, sensor_id, position);
	...
	if (position == SENSOR_5E9_POSITION_FRONT) {
		probe_info("%s setting front cis client\n", __func__);
...
static const struct of_device_id exynos_fimc_is_cis_5e9_match[] = {
	{
		.compatible = "samsung,exynos5-fimc-is-cis-5e9-rear",
		.data = (void *)SENSOR_REAR_DATA
	},
	{
		.compatible = "samsung,exynos5-fimc-is-cis-5e9-front",
